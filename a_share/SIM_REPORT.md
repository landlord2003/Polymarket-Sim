...
<!-- LIVE_DATA_START -->
截至 2026-08-28 23:41（方法论版本 v20260828_234122，自动刷新）：
| 指标 | 数值 | 说明 |
|---|---|---|
| 虚拟本金（账面现金） | **$9941.57** | 相对初始 $10,000 累积（含未实现持仓市值更高） |
| MM 累计实现盈亏 | **$188.23** | 含滑点/漂移后真实口径 |
| MM 胜率 / 净胜率 | 100% / 98% | 0 亏损笔（结构性，见下方警示） |
| 累计滑点成本 | $1.49 | 薄市场走簿产生 |
| **门控跳过 — 深度** | 0 笔 | 深度门槛（市场流动性充足） |
| **门控跳过 — 时间衰减** | 119 笔 | `min_time_to_settle_h=6h` 临近结算硬门控 |
| **门控跳过 — 单市场日上限** | 21 笔 | `daily_cap_notional=$500/24h` 滚动窗口 |
| 纯套利候选 | 230 个 | 平均 edge 0.8596，成交率 100%，**全部待审核角色判定** |
| 纯套利残余库存 | - 份 | 腿风险实证信号 |
| 累计逐笔记录 / 做市执行 | 0 / 90 | 本轮截至自动刷新时刻 |
<!-- LIVE_DATA_END -->
