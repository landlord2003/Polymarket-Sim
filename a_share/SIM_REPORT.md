...
<!-- LIVE_DATA_START -->
截至 2026-08-29 05:40（方法论版本 v20260829_054039，自动刷新）：
| 指标 | 数值 | 说明 |
|---|---|---|
| 虚拟本金（账面现金） | **$9866.78** | 相对初始 $10,000 累积（含未实现持仓市值更高） |
...
| **门控跳过 — 时间衰减** | 4 笔 | `min_time_to_settle_h=6h` 临近结算硬门控 |
| **门控跳过 — 单市场日上限** | 0 笔 | `daily_cap_notional=$500/24h` 滚动窗口 |
| 纯套利候选 | 20 个 | 平均 edge 0.8778，成交率 100%，**全部待审核角色判定** |
| 纯套利残余库存 | - 份 | 腿风险实证信号 |
...
| 候选真实性（数据核验） | 32 候选经数据核验全为独立二元盘拼凑（假 Dutch Book，留门控），当前实盘无错价真划分 |
| 盲区（event_id 聚合≥3二元盘事件数） | 30 |

_审计员签署：real-auditor(独立审计员) ｜ 2026-08-29 08:39_
<!-- AUDIT_SUMMARY_END -->
