...
**直接回答：当前不能保证盈利。模拟验证的是"风控纪律不崩"，不是"盈利能力"。**

- 我们 100% 模拟、0 真实成交样本 → 严格说**盈利几率未知**。
- 行业常识：Polymarket 上**纯价差做市**是负期望游戏（maker 返佣有限、taker 费、逆向选择、库存风险），多数业余做市者净亏。
- 我们的差异化：
  1. **三重合规过滤**砍掉 ~278/300 政治/地缘/军事市场 → 只做 ~22 个安全池，规避黑天鹅但不增加 alpha；
  2. **组合级自动熔断** → 保证不会一把归零，但也不保证赚钱；
  3. **即将做的 LP 奖励套利（#143）**：Polymarket LP 有持币时间加权奖励，是真实正向现金流。模拟里**还没算这笔**，NB 有网才能回填真实奖励半宽 δ。
- **诚实下注**：接真钱后 6 个月**净盈利概率 ~30%（区间 10%–50%）**，强依赖两件事——
  - #143 LP 奖励套利跑通（把双边收益从"纯价差"扩到"价差 + 奖励"）；
  - 真实成交率标定（FILL_BASE 用 NB 实测值，不是 0.30 假设）。
- **最大风险不是亏光本金**（有熔断），而是**薄利被费率/逆向选择/库存风险吃光**，长期在零点附近挣扎。
- **拿到 NB 实盘 1–3 个月真实数据前，任何更精确胜率都是猜的。**
