...
3. **LP 奖励套利（#143，已落地模拟验证）**：Polymarket LP 有持币时间加权奖励 δ，是**真实正向现金流**。已在 `lp_reward.py` 实现「奖励半宽 δ 感知定价」——机会扫描层输出奖励区 `[mid-δ, mid+δ]`、双边是否在带内、价差+奖励的 blended edge、以及把报价压进带内的最优 half_spread；`backtest_lp_reward.py` 做 walk-forward 对比（纯价差 vs 价差+奖励）。**注意：δ 与奖励年化率仍是假设值（北京无外网），NB 有网才能回填真实值**。
...
### 4.1 关于「7 成要亏钱」的精确含义（用户追问澄清）
- **70% = 6 个月跑下来「不赚钱」（净收益 ≤ 0）的概率**，是 30% 的严格补集。
- 这 70% 里**绝大多数不是「亏大钱」**，而是：薄利被费率/逆向选择吃光，净收益在 **0 附近或微亏**；因为组合级熔断（`DRAWDOWN_LIMIT=0.15` / `BANKROLL_FLOOR_FRAC=0.70`）锁死下行，**本金不会归零**，没有「一把亏光」那一档。
- 所以更准确的说法：**7 成不赚钱（含微亏 + 持平），其中微亏为主、大亏几乎被熔断排除**。
- 30% 本身是中值估计（区间 10%–50%），悲观情形（#143 没跑通 / 真实成交率低于 0.30 假设 / 逆向选择重）下盈利概率能掉到 **10% → 即 9 成不赚钱**。
- 盈亏不对称：赚的那 30% 多半是**微赚**（薄利生意），不赚的 70% 是微亏/持平 → 期望值（EV）大概率接近 0 或略负，除非 #143 的 **LP 奖励套利**真把双边收益从「纯价差」扩成「价差 + 奖励」才把 EV 拉正。
